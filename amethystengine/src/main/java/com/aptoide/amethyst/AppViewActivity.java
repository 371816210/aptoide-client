...
import com.aptoide.amethyst.model.AppSecurityBalloon;
...
public class AppViewActivity extends AptoideBaseActivity implements AddCommentVoteCallback,
	...
	protected void onResume() {
		...
		if (fragment != null && resultBundle != null) {
			fragment.onActivityResult(resultBundle.requestCode, resultBundle.resultCode,
					resultBundle.data);
			resultBundle = null;
		}
	...
	public void searchForTrustedApp() {
		final Fragment fragment = getSupportFragmentManager().findFragmentById(R.id.content);
		if (fragment != null) {
			((AppViewFragment) fragment).searchForTrustedApp();
		}
	}
	...
	public static class AppViewFragment extends Fragment implements FlagApkDialog.ApkFlagCallback,
			AddCommentVoteCallback, ServiceConnection {
		...
		/**
		 * Used to control when we need to refresh the install/open button when coming back from
		 * the
		 * onResume
		 */
		...
		private AppSecurityBalloon mSecurityBalloon;
		private RequestListener<GetAppModel> listener = new RequestListener<GetAppModel>() {
			...
			public void onRequestSuccess(GetAppModel model) {
				...
				try {
					...
					storeSubscribers = model.getApp.nodes.meta.data.store.stats.subscribers
							.longValue();
					storeTheme = EnumStoreTheme.get(model.getApp.nodes.meta.data.store.appearance
							.theme);
					...
					if (model.getApp.nodes.versions != null && !model.getApp.nodes.versions.list
							.isEmpty() &&
							model.getApp.nodes.meta.data.file.vercode.longValue() < model.getApp
									.nodes.versions.list
									.get(0).file.vercode.longValue()) {
				...
				if (!unrecoverableErrorsFound) {
					...
					if (!fromSponsored) {
						new AppViewMiddleSuggested((AppViewActivity) getActivity(), getView()
								...
								packageName, model.getApp.nodes.meta.data.media.keywords);
						getOrganicAds();
					}
					...
					if (resultBundle != null) {
						onActivityResult(resultBundle.requestCode, resultBundle.resultCode,
								resultBundle.data);
						resultBundle = null;
					}
			...
			if ((isApkFy || isShowAutoInstallPopup) && !isPaidApp()) {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
					SharedPreferences.Editor edit = PreferenceManager.getDefaultSharedPreferences
							(Aptoide
							.getContext()).edit();
					edit.putString(AptoideConfiguration.PREF_PATH_CACHE_APK, Aptoide.getContext()
							.getFilesDir()
							.getPath()).apply();
				}
		...
		private RequestListener<GetApkInfoJson> getApkInfoJsonRequestListener = new
				RequestListener<GetApkInfoJson>() {
			...
			public void onRequestSuccess(GetApkInfoJson getApkInfoJson) {
				...
				Logger.d("AppViewActivity", "User_ID: " + SecurePreferences.getInstance()
						.getInt("User_ID", 0));
				...
				if (getApkInfoJson.getPayment().status.equals("OK")) {
					...
					mButtonInstall.setText(getString(R.string.buy) + " (" + payment.symbol + " " +
							payment.getAmount() + ")");
					mButtonInstall.setVisibility(View.VISIBLE);
					final Activity thisActivity = getActivity();
					if (SecurePreferences.getInstance().getInt("User_ID", 0) == 0) {
						Logger.d("AppViewActivity", "is 0, making request");
						CheckUserCredentialsRequest request = CheckUserCredentialsRequest
								.buildDefaultRequest(thisActivity, SecurePreferences
								.getInstance()
								.getString("access_token", null));
						spiceManager.execute(request, new
								RequestListener<CheckUserCredentialsJson>() {

							@Override
							public void onRequestFailure(SpiceException spiceException) {
								Toast.makeText(Aptoide.getContext(), R.string.error_occured, Toast
										.LENGTH_SHORT)
										.show();
							}

							@Override
							public void onRequestSuccess(CheckUserCredentialsJson
																 checkUserCredentialsJson) {
								if ("OK".equals(checkUserCredentialsJson.getStatus())) {
									SharedPreferences.Editor securePreferences = SecurePreferences
											.getInstance()
											.edit();

									securePreferences.putInt("User_ID", checkUserCredentialsJson
											.getId());
									Logger.d("AppViewActivity", "updating user id to " +
											checkUserCredentialsJson
											.getId());
									securePreferences.commit();
								} else {
									Toast.makeText(Aptoide.getContext(), R.string.error_occured,
											Toast.LENGTH_SHORT)
											.show();
								}
							}
					...
					mButtonInstall.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View v) {
							...
							if (accounts.length > 0) {
								final Intent i = new Intent(thisActivity, PaidAppPurchaseActivity
										.class);
								...
								Toast.makeText(Aptoide.getContext(), "You need to login to " +
										"purchase.", Toast.LENGTH_LONG)
										.show();
								accountManager.addAccount(Aptoide.getConfiguration()
										.getAccountType(), AptoideConfiguration.AccountGeneral
										.AUTHTOKEN_TYPE_FULL_ACCESS, null, null, thisActivity, new
										AccountManagerCallback<Bundle>() {
									@Override
									public void run(AccountManagerFuture<Bundle>
															bundleAccountManagerFuture) {
		...
		@Nullable
		@Override
		public View onCreateView(final LayoutInflater inflater, @Nullable final ViewGroup
		...
		public void onStart() {
			...
			getActivity().bindService(new Intent(getActivity(), DownloadService.class), this,
					BIND_AUTO_CREATE);
		}
...
//            Aptoide.getRefWatcher(getContext()).watch(this);
		...
		public void onSaveInstanceState(Bundle outState) {
			...
			outState.putBoolean(Constants.SECURITY_BALLOON_TOGGLED, securityBalloonToggled);
		}
		...
		public void onActivityResult(int requestCode, int resultCode, Intent data) {
			...
			if (requestCode == DOWNGRADE_REQUEST_CODE) {
				try {
					...
					Toast.makeText(getActivity(), getString(R.string.downgrade_requires_uninstall)
							, Toast.LENGTH_SHORT)
							.show();
				} catch (PackageManager.NameNotFoundException e) {
				...
				Logger.d("AppViewActivity", "AppViewActivity onActivityResult " +
		...
		private void populateSeeMore() {
			if (description != null && description.length() > 250) {
				try {
					// Fix for AN-348: replace the & with &amp; (that's was causing the pushback
					// buffer full)
		...
		private void populateRatings(GetApp getApp) {
			if (getApp.nodes == null || getApp.nodes.meta == null || getApp.nodes.meta.data ==
					null || getApp.nodes.meta.data.stats == null || getApp.nodes.meta.data.stats
					.rating == null) {
				return;
			}
			...
			long numberVotes = ratings.votes.get(0).count.longValue() + ratings.votes.get(1).count
					.longValue() + ratings.votes
					.get(2).count.longValue() + ratings.votes.get(3).count.longValue() + ratings
					.votes
					.get(4).count.longValue();
				...
				mWebsiteLabel.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						if (AptoideUtils.Algorithms.copyToClipBoard(getActivity(), website)) {
							Toast.makeText(getActivity(), R.string.website_copied, Toast
									.LENGTH_SHORT)
									.show();
						}
					}
				...
				mEmailLabel.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						if (AptoideUtils.Algorithms.copyToClipBoard(getActivity(), email)) {
							Toast.makeText(getActivity(), R.string.email_copied, Toast
									.LENGTH_SHORT)
									.show();
						}
					}
				...
				mPrivacyLabel.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						if (AptoideUtils.Algorithms.copyToClipBoard(getActivity(), privacyAux)) {
							Toast.makeText(getActivity(), R.string.privacy_copied, Toast
									.LENGTH_SHORT)
									.show();
						}
					}
...
//        }
			...
			mWebsiteLabel.setText(Html.fromHtml(AptoideUtils.StringUtils.getFormattedString
					(getActivity(), R.string.website, websiteAux)));
			mEmailLabel.setText(Html.fromHtml(AptoideUtils.StringUtils.getFormattedString
					(getActivity(), R.string.email, emailAux)));
			mPrivacyLabel.setText(Html.fromHtml(AptoideUtils.StringUtils.getFormattedString
					(getActivity(), R.string.privacy_policy, privacyAux)));
			if (getApp == null || getApp.nodes == null || getApp.nodes.meta == null || getApp
					.nodes.meta.data == null || getApp.nodes.meta.data.file == null || getApp
					.nodes.meta.data.file.usedPermissions == null || getApp.nodes.meta.data.file
					.usedPermissions
					.isEmpty()) {
				...
				mPermissionsLabel.setOnClickListener(new View.OnClickListener() {
					@Override
					public void onClick(View v) {
						DialogPermissions dialogPermissions = DialogPermissions.newInstance
								(getApp, appName, versionName, iconUrl, AptoideUtils.StringUtils
								.formatBits(AptoideUtils.AppUtils.sumFileSizes(fileSize, obb)));
				...
				if (get != null && get.list != null) {
					...
					if (get.list.size() > MAX_VISIBLE_COMMENTS) {
						...
						mSeeMoreComments.setOnClickListener(new View.OnClickListener() {
							...
							public void onClick(View v) {
								...
								i.putExtra(Constants.EVENT_LABEL, appName); // The label is used
					...
					if (!get.list.isEmpty()) {
						...
						for (int i = 0; i < get.list.size() && i < MAX_VISIBLE_COMMENTS; i++) {
							displayableCommentsList.add(LatestCommentsFragment.createComment(get
									.list
									.get(i)));
						}

						displayableCommentsList = LatestCommentsFragment.sortComments
								(displayableCommentsList);
					} else {
						displayableCommentsList.add(new NoCommentPlaceHolderRow(bucketSize));
					}

					CommentsStoreAdapter adapter = new CommentsStoreAdapter
							(displayableCommentsList, getActivity(), getResources()
							.getColor(EnumStoreTheme.APTOIDE_STORE_THEME_DEFAULT.getStoreHeader())
									, true, addCommentCallback);
		...
		AddCommentCallback addCommentCallback = new AddCommentCallback() {
			...
			public void addComment(String comment, String answerTo) {
				if (comment != null && comment.length() < Constants.MIN_COMMENT_CHARS) {
					Toast.makeText(getActivity(), R.string.error_IARG_100, Toast.LENGTH_LONG)
							.show();
					return;
				}
				...
				AptoideDialog.pleaseWaitDialog()
						.show(getChildFragmentManager(), "pleaseWaitDialog");
			}
		};

		RequestListener<GenericResponseV2> addCommentRequestListener = new
				AlmostGenericResponseV2RequestListener() {
			@Override
			public void CaseOK() {
				Toast.makeText(getActivity(), getString(R.string.comment_submitted), Toast
						.LENGTH_LONG)
						.show();
				refresh();
				dismissDialog();
			...
			protected void dismissDialog() {
				DialogFragment pd = (DialogFragment) getChildFragmentManager().findFragmentByTag
						("pleaseWaitDialog");
				if (pd != null) {
		...
		private void toggleSecurityBalloon() {
			if (securitydBalloonLayout.getVisibility() == View.VISIBLE) {
				animateSecurityBalloon(securityBalloonFadeOutAnimation);
				securityBalloonToggled = false;
			} else {
				animateSecurityBalloon(securityBalloonFadeInAnimation);
				securityBalloonToggled = true;
			}
		}
		...
		private void animateSecurityBalloon(final Animation animation) {
			securitydBalloonLayout.post(new Runnable() {
				@Override
				public void run() {
					securitydBalloonLayout.startAnimation(animation);
				}
			});
		}
		...
		private void setSecurityBalloonAnimationListeners() {
			final Animation.AnimationListener listener = new Animation.AnimationListener() {

				@Override
				public void onAnimationStart(Animation animation) {
					securitydBalloonLayout.setVisibility(View.VISIBLE);
				}

				@Override
				public void onAnimationEnd(Animation animation) {
					securitydBalloonLayout.setVisibility(View.INVISIBLE);
				}
			...
			securityBalloonStartUpAnimation.setAnimationListener(listener);
			securityBalloonFadeOutAnimation.setAnimationListener(listener);
			securityBalloonFadeInAnimation.setAnimationListener(new Animation.AnimationListener() {
				@Override
				public void onAnimationStart(Animation animation) {
					securitydBalloonLayout.setVisibility(View.VISIBLE);
				}
		...
		private void removeSecurityBalloonAnimationListeners() {
			securityBalloonStartUpAnimation.setAnimationListener(null);
			securityBalloonFadeOutAnimation.setAnimationListener(null);
		...
		DialogInterface.OnDismissListener onDismissListener = new DialogInterface
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if (!new AptoideDatabase(Aptoide.getDb()).existsRepo(storeName)) {
					AptoideDialog.addMyAppStore("http://" + storeName + ".store.aptoide.com/",
							appsAddStoreInterface)
			...
			public DialogInterface.OnClickListener getOnMyAppAddStoreListener(final String repo) {
				return new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						AptoideUtils.RepoUtils.startParse(store.getName(), getActivity(),
								spiceManager);
					}
				};
			}
		...
		private void updateFlags(GetApp getApp) {
			...
			if (getApp == null || getApp.nodes == null || getApp.nodes.meta == null || getApp
					.nodes.meta.data == null || getApp.nodes.meta.data.file == null || getApp
					.nodes.meta.data.file.flags == null) {
				return;
			}
			...
			if (GetAppMeta.File.Flags.GOOD.equals(flags.review)) {
				...
				mButtonFlagThisApp.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						if (AptoideUtils.AccountUtils.isLoggedIn(getActivity())) {
							AptoideDialog.flagAppDialog(userVote)
									.show(getChildFragmentManager(), "flagAppDialog");
						} else {
				...
				if (flags.votes != null) {
					for (GetAppMeta.File.Flags.Vote vote : flags.votes) {
						...
						} else if (vote.type.toUpperCase()
								.equals(GetAppMeta.File.Flags.Vote.FREEZE)) {
							tvNumberFreezeFlags.setText(String.valueOf(numberVotes));
						} else if (vote.type.toUpperCase()
								.equals(GetAppMeta.File.Flags.Vote.FAKE)) {
							tvNumberFakeFlags.setText(String.valueOf(numberVotes));
						} else if (vote.type.toUpperCase()
								.equals(GetAppMeta.File.Flags.Vote.LICENSE)) {
							tvNumberLicenceFlags.setText(String.valueOf(numberVotes));
						} else if (vote.type.toUpperCase()
								.equals(GetAppMeta.File.Flags.Vote.VIRUS)) {
		...
		private void showBadges() {
			...
			if (malware.reason != null) {
				...
				if (malware.reason.signatureValidated != null && GetAppMeta.File.Malware.PASSED
						.equals(malware.reason.signatureValidated.status)) {
					mBadgeSignature.setVisibility(View.VISIBLE);
				}

				if (malware.reason.scanned != null && GetAppMeta.File.Malware.PASSED.equals
						(malware.reason.scanned.status)) {
					mBadgeAntiVirus.setVisibility(View.VISIBLE);
				}

				if (malware.reason.manualQA != null && GetAppMeta.File.Malware.PASSED.equals
						(malware.reason.manualQA.status)) {
		...
		private void updateStoreInfo() {
			...
			if (Build.VERSION.SDK_INT > 10) {
				mButtonSubscribe.setBackgroundDrawable(getResources().getDrawable(storeTheme
						.getButtonLayout()));
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			...
			if (subscribed) {
				...
				mButtonSubscribe.setCompoundDrawablesWithIntrinsicBounds(checkmarkDrawable, 0, 0,
						0);
				...
				mButtonSubscribe.setCompoundDrawablesWithIntrinsicBounds(plusMarkDrawable, 0, 0,
						0);
		...
		class OpenStoreOnClickListener implements View.OnClickListener {
			...
			public void onClick(View v) {
				reloadButtons = true;
				final Context context = getActivity();
				final Intent intent = StoresActivity.newIntent(context, storeId, storeName,
		...
		private void updateUI() {
			...
			mFileSize.setText(AptoideUtils.StringUtils.formatBits(AptoideUtils.AppUtils
					.sumFileSizes(fileSize, obb)));
		...
		private void populateMoreVersions(GetAppModel model) {
			 ...
			 */
			if (getApp == null || getApp.nodes == null || getApp.nodes.versions == null || getApp
					.nodes.versions.list == null || getApp.nodes.versions.list
					.size() < 2) {
			...
			if (!model.list.isEmpty() && model.list.get(0).versionCode == versionCode) {
			...
			final short recyclerViewOffset = (short) TypedValue.applyDimension(TypedValue
					.COMPLEX_UNIT_DIP, 5, getResources()
					.getDisplayMetrics());
			mMoreVersionsList.addItemDecoration(new RecyclerView.ItemDecoration() {
				@Override
				public void getItemOffsets(Rect outRect, View view, RecyclerView parent,
										   RecyclerView.State state) {
					outRect.set(recyclerViewOffset, recyclerViewOffset, recyclerViewOffset,
							recyclerViewOffset);
				}
			});
			...
			gridLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
				...
				public int getSpanSize(int position) {
					...
					if (!(mMoreVersionsList.getAdapter() instanceof SpannableRecyclerAdapter)) {
						throw new IllegalStateException("RecyclerView adapter must extend " +
								"SpannableRecyclerAdapter");
					}
		...
		private void changebtInstalltoBuy() {
			...
			GetApkInfoRequestFromId getApkInfoRequestFromId = new GetApkInfoRequestFromId
					(getActivity());
...
//                mButtonInstall.setVisibility(View.GONE);
				...
				if (info == null) {
					...
					isInstalled = true;
					try {
						installedSignature = AptoideUtils.Algorithms.computeSHA1sumFromBytes(info
								.signatures[0]
								.toByteArray()).toUpperCase(Locale.ENGLISH);
...
//                    UpdateAppVersionInstalled(info.versionName);
						...
						mButtonInstall.setOnClickListener(new DowngradeListener(iconUrl, appName,
								info.versionName, versionName, info.packageName));
//                    UpdateAppVersionInstalled(info.versionName);
					} else {
						changebtInstalltoOpen();
					}
		...
		private void changebtInstalltoOpen() {
			final Intent i = getActivity().getPackageManager()
			...
			spiceManager.execute(getAdsRequest, new RequestListener<ApkSuggestionJson>() {
				...
				public void onRequestSuccess(ApkSuggestionJson apkSuggestionJson) {
					try {
						...
						String clickUrl = appSuggested.getPartner().getPartnerData()
								.getClick_url();
						...
						ReferrerUtils.extractReferrer(packageName, appId, adId, -1, clickUrl,
								spiceManager, null, ReferrerUtils.RETRIES);

						OkHttpClient client = new OkHttpClient();

						Request cpc_click = new Request.Builder().url(appSuggested.getInfo()
								.getCpc_url()).build();
		...
		public void refreshStoresEvent(OttoEvents.RepoAddedEvent event) {
			Toast.makeText(getActivity(), AptoideUtils.StringUtils.getFormattedString(getActivity
		...
		/**
		 * Hides the layout with the install button and shows the download progress bar. Creates a
		 * {@link Download} object and send it to the {@link DownloadService}
		 */
			...
			if (download != null && download.getId() == downloadId) {
				...
				switch (download.getDownloadState()) {
					case ACTIVE:
						...
						mProgressText.setText(download.getProgress() + "% - " + AptoideUtils
								.StringUtils
								.formatBits((long) download.getSpeed()) + "/s");
						break;
					case INACTIVE:
						break;
					case COMPLETE:
		...
		private void setShareTimeLineButton() {
			...
			if (Preferences.getBoolean(Preferences.TIMELINE_ACEPTED_BOOL, false)) {
				...
				btinstallshare.setChecked(Preferences.getBoolean(Preferences
						.SHARE_TIMELINE_DOWNLOAD_BOOL, true));
				btinstallshare.setOnCheckedChangeListener(new CompoundButton
						.OnCheckedChangeListener() {
					@Override
					public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
						final Map<String, String> shareTimelineParams = new HashMap<String,
								String>();
						shareTimelineParams.put("Share_Timeline", String.valueOf(isChecked));
						FlurryAgent.logEvent("App_View_Clicked_On_Share_Timeline_Checkbox",
								shareTimelineParams);
						Preferences.putBooleanAndCommit(Preferences.SHARE_TIMELINE_DOWNLOAD_BOOL,
								isChecked);
					}
				});
			} else {
		...
		private void populateDownloadUI() {
			...
			getView().findViewById(R.id.ic_action_resume)
					.setOnClickListener(new View.OnClickListener() {
						@Override
						public void onClick(View v) {
			...
			if (downloadService != null && downloadService.getDownload(downloadId)
					.getDownload() != null) {
				onDownloadUpdate(downloadService.getDownload(downloadId).getDownload());
			}
		...
		private void populateScreenShotsView() {
			...
			mScreenshotsList.addItemDecoration(new DividerItemDecoration(AptoideUtils.getPixels
					(getActivity(), 5)));
			mScreenshotsList.setLayoutManager(new LinearLayoutManager(getActivity(),
					LinearLayoutManager.HORIZONTAL, false));
			mScreenshotsList.setNestedScrollingEnabled(false); // because otherwise the AppBar
			// won't be collapsed
		...
		private void handleErrorCondition(Exception e) {
			...
			if (e != null && e.getCause() != null && e.getCause()
					.getMessage() != null && e.getCause().getMessage().equals(APP_NOT_AVAILABLE)) {
				...
				mButtonUninstall.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						getFragmentManager().beginTransaction()
								.add(uninstallFragment, "uninstallFrag")
								.commit();
					}
				});
			} else {
		...
		public void addApkFlagClick(String flag) {
			...
			userVote = flag;
			spiceManager.execute(AptoideUtils.RepoUtils.buildFlagAppRequest(storeName, flag,
					md5sum), new RequestListener<GenericResponseV2>() {
				@Override
				public void onRequestFailure(SpiceException spiceException) {
		...
		public void voteComment(int commentId, AddApkCommentVoteRequest.CommentVote vote) {
			RequestListener<GenericResponseV2> commentRequestListener = new
					AlmostGenericResponseV2RequestListener() {
				@Override
				public void CaseOK() {
					Toast.makeText(getActivity(), getString(R.string.vote_submitted), Toast
							.LENGTH_LONG)
							.show();
				}
			};

			AptoideUtils.VoteUtils.voteComment(spiceManager, commentId, storeName,
					SecurePreferences
					.getInstance()
					.getString("token", "empty"), commentRequestListener, vote);
		}

		private void showRootDialog() {
			if (!PreferenceManager.getDefaultSharedPreferences(getActivity())
					.contains("allowRoot") && !Aptoide.IS_SYSTEM) {
						...
						if (AptoideUtils.AppUtils.isRooted()) {
							handler.post(new Runnable() {
								...
								public void run() {
									...
									try {
										AptoideDialog.allowRootDialog()
												.show(getChildFragmentManager(), "allowRoot");
									} catch (Exception e) {
										Logger.printException(e);
									}
								}
		...
		private void requestComments(boolean useCache) {
			long cacheExpiryDuration = (useCache || !forceReload) ? DurationInMillis.ONE_HOUR * 6
					: DurationInMillis.ALWAYS_EXPIRED;

			// call list apkcomments, which need: storeName, packageName, versionName
			// http://webservices.aptoide.com/webservices/2/listApkComments/apps/com.android
			// .vending/5.12.9/json
			...
			spiceManager.execute(request, "comments" + appName + appId + storeName + versionName +
					packageName, cacheExpiryDuration, requestCommentListener);
		}
		...
		private void continueLoading() {
			...
			if (intent.getBooleanExtra(Constants.FROM_RELATED_KEY, false)) {
				...
				if (intent.hasExtra("partnerExtra")) {
					final String clickUrl = intent.getBundleExtra("partnerExtra")
							.getString("partnerClickUrl");
					ReferrerUtils.extractReferrer(packageName, appId, adId, -1, clickUrl,
							spiceManager, null, ReferrerUtils.RETRIES);
				}
		...
		private void executeSpiceRequest() {
			long cacheExpiryDuration = forceReload ? DurationInMillis.ALWAYS_EXPIRED :
			...
			spiceManager.execute(getAppRequest, md5sum + packageName + appId + bucketSize +
					storeName, cacheExpiryDuration, listener);
		}

		private void executeSpiceRequestWithAppId(long appId, @Nullable String storeName, String
				packageName) {
			long cacheExpiryDuration = forceReload ? DurationInMillis.ALWAYS_EXPIRED :
					DurationInMillis.ONE_HOUR;
			spiceManager.execute(AptoideUtils.RepoUtils.buildGetAppRequestFromAppId(appId,
					storeName, packageName), appId + bucketSize, cacheExpiryDuration, listener);
		}

		private void executeSpiceRequestWithMd5(String md5, String storeName) {
			long cacheExpiryDuration = forceReload ? DurationInMillis.ALWAYS_EXPIRED :
					DurationInMillis.ONE_HOUR;
			spiceManager.execute(AptoideUtils.RepoUtils.buildGetAppRequestFromMd5(md5, storeName),
					md5 + bucketSize, cacheExpiryDuration, listener);
		}

		private void executeSpiceRequestWithPackageName(String packageName, String appName) {
			long cacheExpiryDuration = forceReload ? DurationInMillis.ALWAYS_EXPIRED :
					DurationInMillis.ONE_HOUR;
			spiceManager.execute(AptoideUtils.RepoUtils.buildGetAppRequestFromPackageName
					(packageName), packageName + appName + bucketSize, cacheExpiryDuration,
					listener);
		}

		private void executeSpiceRequestWithAppRate(long appId, float rating) {
			mRatingBar.setIsIndicator(true);
			spiceManager.execute(AptoideUtils.RepoUtils.buildRateRequest(appId, rating), new
					RequestListener<RateApp>() {
				@Override
				public void onRequestFailure(SpiceException spiceException) {
					Toast.makeText(getActivity(), getString(R.string.error_occured), Toast
							.LENGTH_SHORT)
							.show();
				...
				public void onRequestSuccess(RateApp rateApp) {
					mRatingBar.setIsIndicator(false);
					Toast.makeText(getActivity(), getString(R.string.appview_rate_Success), Toast
							.LENGTH_SHORT)
							.show();
				}
			});
		...
		public class DowngradeListener implements View.OnClickListener {
			...
			public DowngradeListener(String icon, String name, String versionName, String
					downgradeVersion, String package_name) {
				this.icon = icon;
				this.name = name;
		...
		public class InstallListener implements View.OnClickListener, DialogInterface
				.OnClickListener {
		...
		public void searchForTrustedApp() {
			...
			searchIntent.putExtra(SearchActivity.TRUSTED, true);
